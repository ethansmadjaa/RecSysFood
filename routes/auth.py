...
async def sign_in(credentials: SignInRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Authentication error: {error_message}")
...
async def sign_up(data: SignUpRequest):
    ...
    try:
        ...
        return AuthResponse(
            user=auth_response.user.model_dump() if hasattr(auth_response.user, 'model_dump') else auth_response.user.__dict__,
            session=None,
            error=None
        )
...
async def get_user_profile(auth_id: str):
    ...
    try:
        response = supabase.table('users').select('*').eq('auth_id', auth_id).single().execute()
