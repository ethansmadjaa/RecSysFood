...
def pre_processing(user_preference_db, recipe_db, interaction_db):
    ...
    # Recipe processing
    recipe_features = ['RecipeId','Calories', 'FatContent', 'ProteinContent','SugarContent',
                        'PrepTime_min' ,'TotalTime_min','Is_Vegan','Is_Vegetarian',
                        'Contains_Pork','Contains_Alcohol','Contains_Gluten', 'Contains_Nuts',
                        'Contains_Dairy', 'Contains_Egg','Contains_Fish',
                        'Contains_Soy','Is_Breakfast_Brunch', 'Is_Dessert']
...
def Graph_to_PyG(G, user_preference_db, recipe_db):
    ...
    num_recipe_features = ['Calories', 'FatContent', 'ProteinContent', 'SugarContent'
                  ,'PrepTime_min' ,'TotalTime_min','Is_Vegan', 'Is_Vegetarian',
                  'Contains_Pork', 'Contains_Alcohol','Contains_Gluten', 'Contains_Nuts',
                 'Contains_Dairy', 'Contains_Egg','Contains_Fish', 'Contains_Soy','Is_Breakfast_Brunch', 'Is_Dessert']
...
def training(model, train_data, optimizer, loss):
    ...
    z = model.Encoder(
    x_dict = {
        "user": train_data["user"].x,
        "recipe": train_data["recipe"].x
    },
    edge_index_dict = {
...
def test(model, val_test_data):
    ...
    z = model.Encoder(
    x_dict = {
        "user": val_test_data["user"].x,
        "recipe": val_test_data["recipe"].x
    },
    edge_index_dict = {
...
def Train_GNN(user_preference_db, recipe_db, interaction_db):
    ...
    # Process dataset
    user_preference_db, recipe_db, interaction_db = pre_processing(user_preference_db, recipe_db, interaction_db)
    ...
    # Split the data
    transform = T.RandomLinkSplit(num_val=0.05, num_test=0.05,
                              is_undirected = True,
    ...
    for i in range(epochs):
        ...
        l = training(model, train_data, optimizer=Adam_opt, loss=loss_fct)
...
def Recommendation(model, data, user_id, recipe_db, top_k=10):
    ...
    # Embeddings
    user_emb = out['user'][user_id]    # [hidden_dim]
...
# Load the dataset
user_preference_db = pd.read_csv("C:/Users/Ilian/OneDrive/Documents/GitHub/RecSysFood/models/user_profil.txt")
recipe_db = pd.read_csv("C:/Users/Ilian/OneDrive/Documents/GitHub/RecSysFood/models/recipes.csv")
interaction_db = pd.read_csv("C:/Users/Ilian/OneDrive/Documents/GitHub/RecSysFood/models/interaction.txt")
