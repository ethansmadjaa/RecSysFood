...
columns_to_update = ['images', 'keywords', 'recipeingredientquantities', 'recipeingredientparts']
...
BATCH_SIZE = 1000
SKIP_FIRST = 12000  # Skip the first 12000 recipes
...
for i in range(SKIP_FIRST, total_rows, BATCH_SIZE):
    ...
    print(f"Processing batch {(i - SKIP_FIRST) // BATCH_SIZE + 1}/{(total_rows - SKIP_FIRST) // BATCH_SIZE + 1}")
    for _, row in batch.iterrows():
        ...
        if update_data:
            try:
                supabase.table('recipes').update(update_data).eq('recipeid', int(row['recipeid'])).execute()
